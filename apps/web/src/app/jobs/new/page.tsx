      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
            <Briefcase className="h-6 w-6 text-[#0A66C2]" /> Post Engineering Position
          </h1>
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={cn(
                    ...
                    step > i ? "bg-[#0A66C2]" : "bg-slate-200 dark:bg-slate-700"
                  )}
                />
              )}
            </div>
      ...
      <div className={cn("grid gap-6", showPreview ? "lg:grid-cols-12" : "grid-cols-1")}>
            ...
            {step === 0 && (
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                  <Sparkles className="h-3 w-3 text-[#0A66C2]" /> Quick Start Role Templates:
                </span>
                <div className="flex flex-wrap gap-1.5">
                  <button
                    type="button"
                    onClick={() => applyTemplate("frontend")}
                    className="text-xs bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-2.5 py-1 rounded-lg hover:border-[#0A66C2] font-medium transition-colors"
                  >
                    ⚡ Full-Stack React
                  </button>
                  <button
        ...
        {showPreview && (
          <div className="lg:col-span-5 space-y-4">
            <div className="card-enterprise p-5 space-y-4 sticky top-6 border-[#0A66C2]/30 shadow-md">
