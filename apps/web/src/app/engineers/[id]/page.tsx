    ...
    <ol className="relative space-y-0 border-l-2 border-slate-100 pl-5">
      {items.map((item, i) => (
        <li key={`${item.company}-${i}`} className="relative pb-6 last:pb-0">
          {/* Timeline dot */}
          <span className="absolute -left-[1.4rem] top-0.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-[#0866FF] bg-white">
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 py-8">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-slate-500">
        <Link href="/engineers" className="hover:text-[#0866FF] hover:underline">Professionals</Link>
        <span>/</span>
      ...
      <section className="card-enterprise overflow-hidden">
        {/* Cover banner */}
        <div className="h-28 bg-gradient-to-r from-[#0866FF] via-[#1D6FE0] to-[#1D4ED8]" />
        <div className="px-6 pb-6">
          <div className="-mt-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            {/* Avatar + name */}
            <div className="flex items-end gap-4">
              <div className="flex h-20 w-20 items-center justify-center rounded-2xl border-4 border-white bg-gradient-to-br from-[#0866FF] to-[#0552CC] text-2xl font-bold text-white shadow-md">
                {initials}
              </div>
              <div className="pb-1">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {profile.projects.map((project) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group rounded-xl border border-slate-100 p-4 transition-all hover:border-[#0866FF] hover:shadow-sm"
                  >
                    <h3 className="text-sm font-semibold text-slate-900 group-hover:text-[#0866FF]">
                      {project.title}
                    </h3>
          ...
          {(profile.github_url || profile.linkedin_url || profile.portfolio_url || profile.resume_url) && (
            <section className="card-enterprise p-5">
              ...
              <div className="space-y-2">
                {profile.github_url && (
                  <a href={profile.github_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs text-slate-600 hover:text-[#0866FF]">
                    <Github className="h-4 w-4 shrink-0" /> GitHub profile
                  </a>
                )}
                {profile.linkedin_url && (
                  <a href={profile.linkedin_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs text-slate-600 hover:text-[#0866FF]">
                    <Linkedin className="h-4 w-4 shrink-0" /> LinkedIn
                  </a>
                )}
                {profile.portfolio_url && (
                  <a href={profile.portfolio_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs text-slate-600 hover:text-[#0866FF]">
                    <ExternalLink className="h-4 w-4 shrink-0" /> Portfolio site
                  </a>
                )}
                {profile.resume_url && (
                  <a href={profile.resume_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs font-semibold text-[#0866FF] hover:underline">
