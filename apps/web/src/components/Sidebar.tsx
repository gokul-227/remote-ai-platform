    ...
    <div className="w-full space-y-4">
      ...
      <div className="card-enterprise overflow-hidden text-center">
        ...
        <div className="px-4 pb-4 -mt-7">
          <div className="h-14 w-14 rounded-full bg-[#0866FF] text-white flex items-center justify-center font-bold text-lg ring-4 ring-white mx-auto shadow-xs">
          ...
          {user.role === "ENGINEER" && (
            <div className="border-t border-slate-100 dark:border-slate-800 mt-3 pt-3 text-left text-xs">
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                ...
                <span className="font-semibold text-[#0866FF]">{applications.data?.length ?? 0}</span>
              </div>
            </div>
          )}
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-md text-xs font-medium transition-colors ${
                  isActive
                    ? "bg-slate-100 dark:bg-slate-800 text-[#0866FF] font-semibold"
                    : "text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
                }`}
              >
                <div className="flex items-center gap-2.5">
                  <Icon className={`h-4 w-4 ${isActive ? "text-[#0866FF]" : "text-slate-400"}`} />
                  <span>{item.name}</span>
                </div>
                {item.count !== undefined && (
                  <span className="text-[10px] font-bold text-[#0866FF] bg-sky-50 px-1.5 py-0.5 rounded-full border border-sky-100">
                    {item.count}
                  </span>
                )}
              </Link>
