      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 max-w-xl">
          ...
          <Link href="/" className="flex items-center gap-2 flex-shrink-0">
            <div className="h-9 w-9 rounded-lg bg-[#0866FF] flex items-center justify-center text-white font-black text-xl shadow-xs">
              R
            </div>
          ...
          <form onSubmit={handleSearch} className="flex-1 hidden md:block relative">
            <div className="relative">
              ...
              <input
                ...
                className="input-enterprise pl-10 pr-12 py-1.5 text-xs bg-slate-100 dark:bg-[var(--bg-subtle)] focus:bg-white dark:focus:bg-[var(--bg-surface)] border-transparent focus:border-[#0866FF]"
              />
              <button
                type="button"
                ...
                {search.isLoading ? (
                  ...
                  <div className="py-1">
                    ...
                    <button
                      type="submit"
                      onMouseDown={() => setSearchOpen(false)}
                      className="w-full text-left px-4 py-2 text-xs font-semibold text-[#0866FF] hover:bg-slate-50 border-t border-slate-100"
                    >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="relative">
            ...
            >
              ...
              {(notifications.unread.data?.count ?? 0) > 0 && <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-[#0866FF]" />}
            </button>
            {notifOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-[var(--surface-elevated)] rounded-xl shadow-lg border border-slate-200 dark:border-[var(--border-color)] p-4 z-50 animate-fade-in">
                <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-[var(--border-color)] mb-3">
                  ...
                  <button onClick={() => notifications.markAllRead.mutate()} className="text-xs text-[#0866FF] cursor-pointer hover:underline">Mark all read</button>
                </div>
                <div className="space-y-2.5">
                  {notifications.data?.length ? notifications.data.map((n: { id: string; title: string; body: string; is_read?: boolean }) => (
                    <button onClick={() => notifications.markRead.mutate(n.id)} key={n.id} className={`flex w-full items-start gap-2.5 p-2 rounded-lg text-left hover:bg-slate-50 dark:hover:bg-[var(--bg-subtle)] cursor-pointer ${n.is_read ? "opacity-60" : ""}`}>
                      <div className="h-2 w-2 rounded-full bg-[#0866FF] mt-1.5 flex-shrink-0" />
                      <div>
                ...
                <Link
                  ...
                  className="block text-center text-xs font-semibold text-[#0866FF] hover:underline mt-3 pt-3 border-t border-slate-100 dark:border-[var(--border-color)]"
                >
