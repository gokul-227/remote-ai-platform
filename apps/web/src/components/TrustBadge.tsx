...
export function TrustBadge({ userId, showBreakdownToggle = true }: { userId: string; showBreakdownToggle?: boolean }) {
  ...
  const getScoreTier = (val: number) => {
    ...
    if (val >= 70) return { label: "Verified Professional", color: "bg-[#0552CC] text-white border-[#033D8F]" };
      ...
      <div className="inline-flex items-center gap-2">
        ...
        {showBreakdownToggle && (
          <button
            ...
            className="text-xs text-slate-500 hover:text-[#0552CC] font-semibold inline-flex items-center gap-0.5"
          >
        ...
        <div className="card-enterprise p-4 max-w-sm space-y-3 bg-white shadow-lg border border-slate-200 rounded-xl text-xs text-left">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <span className="font-bold text-slate-900 flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-[#0552CC]" /> Explainable Trust Factors
            </span>
            ...
            {factors.map((factor, idx) => (
              <div key={idx} className="space-y-0.5">
                <div className="flex justify-between font-semibold text-slate-800">
                  ...
                  <span className="text-[#0552CC]">{factor.points} / {factor.max} pts</span>
                </div>
          ...
          <div className="border-t border-slate-100 pt-2 space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              <button
                ...
                disabled={addVerification.isPending}
                className="text-[10px] font-semibold bg-sky-50 text-[#0552CC] hover:bg-sky-100 px-2 py-1 rounded"
              >
