...
APPLICATION_STATUSES = {"SUBMITTED", "REVIEWING", "SHORTLISTED", "REJECTED", "ACCEPTED", "WITHDRAWN", "INVITED", "APPLIED"}
ALLOWED_TRANSITIONS = {
...
async def apply_to_job(
    ...
    application = JobApplication(user_id=current_user.id, job_id=job_id, status="SUBMITTED", cover_note=data.cover_note)
    db.add(application)
    await db.flush()
    return application
...
async def list_company_applications(
    ...
    query = select(JobApplication, JobPost, User, EngineerProfile).join(JobPost, JobPost.id == JobApplication.job_id)
    if company:
...
async def invite_engineer(
    ...
    existing = await db.scalar(select(JobApplication).where(JobApplication.user_id == target_user_id, JobApplication.job_id == job_id))
    if existing:
