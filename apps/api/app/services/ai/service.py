...
class AIService:
    ...
    def __init__(self, model: Optional[str] = None, db: Optional[AsyncSession] = None):
    ...
    async def _record_usage(self, started: float, prompt_key: Optional[str], prompt_version: Optional[str]) -> None:
        if self.db is None:
            return
