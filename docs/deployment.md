# WorkMesh AI — Deployment Guide

This document outlines deployment options and steps for deploying WorkMesh AI to free / developer-friendly cloud services.
...
2. Create a new project named `workmesh-db`.
...
```bash
DATABASE_URL="postgresql+asyncpg://user:pass@ep-xyz.neon.tech/workmesh?ssl=require" alembic upgrade head
```
...
4. Environment Variables:
   - `NEXT_PUBLIC_API_URL`: Your Render backend API URL (e.g. `https://workmesh-api.onrender.com`).
5. Deploy.
...
WorkMesh AI uses standard AWS S3 / MinIO protocol. Set environment variables on the backend:
...
2. Trigger the job seeder to populate production demo data:
   ```bash
   curl -X POST "https://your-backend.onrender.com/api/v1/jobs/seed_demo"
   ```
3. Test browsing jobs, filtering, creating engineer profiles, and AI match scoring.
